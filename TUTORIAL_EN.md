...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://github.com/BruceYuan10/leaf_new/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
A game server implemented with Leaf may include many modules (e.g. [LeafServer](https://github.com/BruceYuan10/leaf_newserver)) which all share below traits:

* Each module runs inside a separate goroutine
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://github.com/BruceYuan10/leaf_new/tree/master/chanrpc))
...
[LeafServer](https://github.com/BruceYuan10/leaf_newserver) is a game server developped with Leaf. Let's start with it.

Download the source code of LeafServer：

```
git clone https://github.com/BruceYuan10/leaf_newserver
```

Download and install leafserver to GOPATH:

```
go get github.com/BruceYuan10/leaf_new
```
...
import (
    "github.com/BruceYuan10/leaf_new/network"
)
...
import (
    "github.com/BruceYuan10/leaf_new/network/json"
)
...
import (
    ...
    "github.com/BruceYuan10/leaf_new/gate"
    "reflect"
    "server/msg"
)
...
import (
    "github.com/BruceYuan10/leaf_new/module"
    "server/base"
)
...
More references are at [leaf/go](https://github.com/BruceYuan10/leaf_new/blob/master/go)。
...
More references are at [leaf/timer](https://github.com/BruceYuan10/leaf_new/blob/master/timer)。
...
More references are at [leaf/log](https://github.com/BruceYuan10/leaf_new/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://github.com/BruceYuan10/leaf_new/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://github.com/BruceYuan10/leaf_new/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://github.com/BruceYuan10/leaf_new/wiki](https://github.com/BruceYuan10/leaf_new/wiki)
